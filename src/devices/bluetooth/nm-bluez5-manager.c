...
_network_server_unregister (NMBluez5Manager *self, NetworkServer *network_server)
{
	...
	nm_log_info (LOGD_BT, "NAP: unregistering %s from %s",
	...
	g_dbus_connection_call (g_dbus_proxy_get_connection (priv->proxy),
	                        BLUEZ_SERVICE,
	                        network_server->path,
	                        BLUEZ5_NETWORK_SERVER_INTERFACE,
	                        "Unregister",
                             ...
                             const char *addr)
{
	...
	ret = !!_find_network_server (self, NULL, addr, NULL);

	nm_log_trace (LOGD_BT, "NAP: %s %s available", addr, ret ? "is" : "is not");
                                ...
                                const char *addr, const char *uuid, NMDevice *device)
{
	...
	if (!network_server) {
		...
		nm_log_info (LOGD_BT, "NAP: %s is not available for %s",
			     addr, nm_device_get_iface (device));
		return FALSE;
	}

	nm_log_info (LOGD_BT, "NAP: registering %s on %s",
	             nm_device_get_iface (device), network_server->addr);
...
network_server_removed (GDBusProxy *proxy, const gchar *path, NMBluez5Manager *self)
{
	...
	nm_log_info (LOGD_BT, "NAP: removed interface %s", network_server->addr);
...
network_server_added (GDBusProxy *proxy, const gchar *path, const char *addr, NMBluez5Manager *self)
{
	...
	nm_log_info (LOGD_BT, "NAP: added interface %s", addr);
                                 ...
                                 NMBluez5Manager *self)
{
	...
	if (g_variant_lookup (dict, BLUEZ5_NETWORK_SERVER_INTERFACE, "a{sv}", NULL)) {
		GVariant *adapter = g_variant_lookup_value (dict, BLUEZ5_ADAPTER_INTERFACE, G_VARIANT_TYPE_DICTIONARY);
		const char *address;
                                   ...
                                   NMBluez5Manager  *self)
{
	...
	if (ifaces && g_strv_contains (ifaces, BLUEZ5_NETWORK_SERVER_INTERFACE))
		network_server_removed (proxy, path, self);
}
